    ...
    <footer className="w-full bg-white border-t-[1.25px] border-[#e5e7eb] mt-auto">
      <div className="flex flex-col md:flex-row items-center justify-between w-full max-w-7xl mx-auto px-[16px] md:px-[31.99px] pt-[15.25px] pb-[14px] gap-4 md:gap-0">
        <button
          onClick={onBack}
          disabled={step === 1}
          className={`text-[#374151] text-[13px] font-medium font-['Inter'] leading-[13px] hover:text-[#111827] transition-colors flex items-center ${
        ...
        <div className="flex flex-col sm:flex-row gap-[7.99px] items-center w-full md:w-auto">
          ...
          <Button
            onClick={onNext}
            className="bg-[#4f46e5] text-white text-[13px] font-medium font-['Inter'] h-auto px-[17.25px] py-[11.25px] rounded-[8px] hover:bg-[#4338ca] transition-colors shadow-none w-full sm:w-auto"
          >
            {isLastStep ? 'Guardar y empezar el diagnóstico →' : 'Continuar (opcional) →'}
          </Button>
        </div>
      </div>
    </footer>
