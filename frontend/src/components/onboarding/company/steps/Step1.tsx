...
export function Step1({ data, onUpdate }: Step1Props) {
  ...
  const updateField = (field: string, value: string) => {
    if (field === 'website') {
      setWebsiteError(value && !isValidUrl(value) ? 'Ingresá una URL válida (ej: https://empresa.com)' : '');
    }
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-[#1a1a2e]">
          Contanos sobre vos y tu empresa
        </h1>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <Input
            ...
            className={ `h-12 border-gray-200 rounded-md ${websiteError ? 'border-red-400' : ''}` }
          />
          { websiteError && (
